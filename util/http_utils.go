...
func BuildQueryStringDelimitedSlices(obj interface{}) string {
    v := reflect.ValueOf(obj)
    t := reflect.TypeOf(obj)
        ...
        if field.Kind() == reflect.Ptr && !field.IsNil() {
            ...
            default:
                if field.Kind() == reflect.Slice {
                    sliceVal := reflect.Indirect(field)
                    if sliceVal.Len() > 0 {
                        var strVals []string
                        for j := 0; j < sliceVal.Len(); j++ {
                            strVals = append(strVals, sliceVal.Index(j).Interface().(fmt.Stringer).String())
                        }
                        val = strings.Join(strVals, ",")
                    } else {
                        continue
                    }
                } else {
